#!/usr/bin/env python3
# This is a copy of the prep_Tamasheq_data.py file from the fairseq library.
# You will need to add this script at fairseq/examples/speech_to_text in order to make it run correctly
...
class Tamasheq(Dataset):
    ...
    def __init__(self, root: str, lang: str, split: str) -> None:
        ...
        for wav_filename, _seg_group in groupby(segments, lambda x: x["wav"]):
            wav_filename = wav_filename +".wav" #.replace(".wav", ".flac")
    ...
    def __getitem__(self, n: int) -> Tuple[torch.Tensor, int, str, str, str, str, str]:
        wav_path, offset, n_frames, sr, tgt_utt, spk_id, tgt_lang, utt_id = self.data[n]
        waveform, _ = get_waveform(wav_path, frames=n_frames, start=offset)
        waveform = torch.from_numpy(waveform)
        return waveform, sr, tgt_utt, spk_id, tgt_lang, utt_id
...
def process(args):
    ...
    for lang in Tamasheq.LANGPAIRS:
        ...
        for split in Tamasheq.SPLITS:
            print(f"Fetching split {split}...")
            dataset = Tamasheq(root.as_posix(), lang, split)
        ...
        for split in Tamasheq.SPLITS:
            ...
            dataset = Tamasheq(args.data_root, lang, split)
            for wav, sr, tgt_utt, speaker_id, tgt_lang, utt_id in tqdm(dataset):
...
def process_joint(args):
    ...
    assert all((cur_root / f"{lang}").is_dir() for lang in Tamasheq.LANGPAIRS), \
    ...
    with NamedTemporaryFile(mode="w") as f:
        ...
        if args.joint:
            ...
            special_symbols = list({f'<lang:{lang.split("-")[1]}>' for lang in Tamasheq.LANGPAIRS})
        gen_vocab(
            Path(f.name),
            cur_root / spm_filename_prefix,
            args.vocab_type,
